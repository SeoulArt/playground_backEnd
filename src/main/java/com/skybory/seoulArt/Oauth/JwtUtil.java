...
//        long expiry = 1000L * 60 * 10; // RefreshToken 유효 시간은 1개월 (ms단위)
	...
	public void setAccessToken(HttpHeaders headers, String accessToken) {
		ResponseCookie accessTokenCookie = ResponseCookie.from("accessToken", accessToken).httpOnly(true).secure(false) // Configurable
																														// based
																														// on
																														// the
																														// deployment
...
//                .maxAge(1000L * 60 * 3)  // 3 분
	...
	public void setRefreshToken(HttpHeaders headers, String jwtRefreshToken) {
		ResponseCookie refreshTokenCookie = ResponseCookie.from("refreshToken", jwtRefreshToken).httpOnly(true)
				.secure(false).path("/")
