...
public class ReviewServiceImpl implements ReviewService {
	...
	@Override
	@Transactional
	public CreateReviewResponse createReview(CreateReviewRequest request, HttpServletRequest requestServlet)
			throws IOException {
...
//		response.setPlayId(request.getPlayId());
	...
	@Override
	@Transactional
	public PutReviewResponse putReview(PutReviewRequest request, HttpServletRequest requestServlet, Long reviewId)
	        throws IOException {
...
//	    review.setPlay(playRepository.findById(request.getPlayId()).orElseThrow(() -> new ServiceException(ErrorCode.PLAY_NOT_FOUND))); // 연극 설정
	    ...
	    // 응답 생성
	    PutReviewResponse response = new PutReviewResponse();
	    response.setReviewId(review.getReviewId());
...
//	    response.setPlayId(review.getPlay().getPlayId());
	...
	@Override
	public List<ReviewListResponse> showAll(Long playId) {
...
//		List<Review> replies = reviewRepository.findAll();
		...
		List<ReviewListResponse> responseList = replies.stream().map(review -> {
...
//	    else if (role.equals("ROLE_USER") | role.equals("ROLE_CREATOR")) {
//	        if (review.getUser().getId().equals(userId)) {
//	            try {
//	                reviewRepository.deleteById(reviewId);
//	                return "후기 삭제 성공"; // 삭제 성공
